...
// ── Module-level helpers (component-এর বাইরে) ────────────
// এরা কোনো state/prop এর উপর depend করে না
// তাই এখানে রাখলে useCallback dependency সমস্যা হবে না
...
const convertFileToWebP = async (file: File, quality = 0.88): Promise<Blob> => {
...
const ImageUploadWithEditor = ({
  ...
  // ── Preview → Confirm ────────────────────────────────────
  // convertFileToWebP module-level এ আছে তাই dependency লাগবে না
    ...
    try {
      const webpBlob =
        previewFile.type === "image/webp"
          ? previewFile
          : await convertFileToWebP(previewFile, 0.88);

      const newPreviewUrl = URL.createObjectURL(webpBlob);
      ...
      console.error("Preview confirm WebP conversion failed:", err);
    }
  }, [previewFile, previewUrl, images, onChange, openNextPreview]);
  // ✅ convertFileToWebP এখন module-level তাই এখানে লাগবে না
